...
with st.sidebar:
    ...
    if not summary_df.empty:
        ...
        total_tasks = 50 * 7  # 50 days * 7 universities
...
if menu == "Dashboard":
    ...
    if not summary_df.empty:
        ...
        with col3:
            st.metric("🔄 In Progress", f"{summary_df['In Progress'].sum()}")
        with col4:
    ...
    current_day = (datetime.now() - datetime(2024, 11, 1)).days + 1
    if 1 <= current_day <= 50:
    ...
    selected_uni = st.selectbox("University", list(UNIVERSITIES.keys()), format_func=lambda x: UNIVERSITIES[x]["name"], key="update_uni")
    ...
    if selected_uni:
        ...
        if not pending_df.empty:
            selected_day = st.selectbox("Select Task", pending_df["Day"].tolist(), format_func=lambda x: f"Day {x}: {pending_df[pending_df['Day']==x]['Task'].iloc[0]}")
    ...
    report_type = st.selectbox("Report Type", ["Summary Report", "Framework Report", "Export Data"])
